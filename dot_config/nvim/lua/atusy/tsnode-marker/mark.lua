...
local function _indent(buf, node, lines, tabstop, opts)
  ...
  if o == nil or o == "node" then
    return require("atusy.tsnode-marker.indent").measure_common_indent(lines, tabstop)
  end
...
local function is_target(buf, node, opts)
  local _is_target = opts.is_target
  local _type = type(_is_target)
  if _type == "function" then
    return _is_target(buf, node)
  end
  if _type == "table" then
    return vim.tbl_contains(_is_target, node:type())
  end
  return false
end
...
function M.mark_nodes_in_range(buf, opts)
  ...
  local tsnode = require("atusy.tsnode-marker.tsnode")
