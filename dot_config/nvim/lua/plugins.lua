  ...
  },
  {
    ...
    init = function(p)
      ...
      vim.keymap.set({ "n", "x", "o" }, leader .. ";", function()
        require("flash").jump({
          labels = "ASDFGHJKLQWERTYOPZXCVBNM",
          label = { before = { 0, 0 }, after = false },
          search = {
            multi_window = false,
            mode = function(str)
              local pattern = "\\c" .. str
              local kensaku = vim.fn["kensaku#query"](str)
              local wininfo = vim.fn.getwininfo(vim.api.nvim_get_current_win())[1]
              local prefix = [[\%>]] .. (wininfo.topline - 1) .. [[l\%<]] .. (wininfo.botline + 1) .. "l"
              return prefix .. pattern .. "\\|" .. kensaku
            end,
          },
        })
      end)
